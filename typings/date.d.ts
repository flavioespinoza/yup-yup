...
declare module 'yup' {
  ...
  interface DateSchema<T extends string | null | undefined = string> extends MixedSchema<T> {
    min(date: string, errorMessage?: string): DateRangeSchema<T>;
    max(date: string, errorMessage?: string): DateRangeSchema<T>;
    between(minDate: string, maxDate: string, errorMessage?: string): DateSchema<T>;
